...
namespace DeliveryApp.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("DeliveryApp") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "DeliveryApp", DisplayName = "DeliveryApp API", Resources = { "DeliveryApp" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["DeliveryApp_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["DeliveryApp_Web:RootUrl"]!.EnsureEndsWith('/');

            /* DeliveryApp_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["DeliveryApp_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // SPA Client (Angular dashboard)
        var spaClientId = configurationSection["DeliveryApp_App:ClientId"];
        if (!spaClientId.IsNullOrWhiteSpace())
        {
            var spaRootUrl = configurationSection["DeliveryApp_App:RootUrl"]?.TrimEnd('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["DeliveryApp_App:ClientSecret"] ?? "YXJzdRf2yF8bjY4iIvNc8fn6VQSR5nwGWLZfkvsErfKVEOI5hu6tcyh8uvfjjUmP",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["DeliveryApp_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["DeliveryApp_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        List<string>? permissions = null)
    {
        ...
        // Update redirect/post-logout URIs if they have changed
        ...
        // Update scopes/permissions if they have changed
        ...
        // Ensure the stored secret is either hashed-and-valid or update it via the manager (which re-hashes)
        if (!string.IsNullOrEmpty(application.ClientSecret))
        {
            ...
            {
                var verification = passwordHasher.VerifyHashedPassword(null, client.ClientSecret, application.ClientSecret);
                if (verification == Microsoft.AspNetCore.Identity.PasswordVerificationResult.Failed)
                {
                    // stored secret is either plaintext or hash of different value
                    needsUpdate = true;
                }
            }
            ...
            if (needsUpdate)
            {
                ...
                await _applicationManager.UpdateAsync(client.ToModel()); // UpdateAsync re-hashes secret
            }
        }
        return;
    }
