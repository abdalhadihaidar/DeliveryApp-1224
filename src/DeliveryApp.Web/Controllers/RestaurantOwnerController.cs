...
namespace DeliveryApp.Web.Controllers
{
    ...
    public class RestaurantOwnerController : AbpController
    {
        ...
        private readonly IRestaurantOwnerNotificationService _notificationService;

        public RestaurantOwnerController(
            IRestaurantOwnerAppService restaurantOwnerAppService,
            IRestaurantOwnerNotificationService notificationService)
        {
        ...
        /// </summary>
        [HttpPost("restaurants/{restaurantId}/notifications/send")]
        public async Task<NotificationCampaignDto> SendNotificationToCustomers(Guid restaurantId, [FromBody] SendRestaurantNotificationDto dto)
        {
            var userId = GetCurrentUserId();
            dto.RestaurantId = restaurantId;
            return await _notificationService.SendNotificationToCustomersAsync(dto, userId);
        }

        /// <summary>
        /// Get previous customers of a restaurant with filtering options
        /// </summary>
        [HttpGet("restaurants/{restaurantId}/customers")]
        public async Task<Application.Contracts.Dtos.PagedResultDto<RestaurantCustomerDto>> GetRestaurantCustomers(Guid restaurantId, [FromQuery] GetRestaurantCustomersDto dto)
        {
            var userId = GetCurrentUserId();
            dto.RestaurantId = restaurantId;
            return await _notificationService.GetRestaurantCustomersAsync(dto, userId);
        }

        /// <summary>
        /// Get customer segments for a restaurant
        /// </summary>
        [HttpGet("restaurants/{restaurantId}/customer-segments")]
        public async Task<List<DeliveryApp.Application.Contracts.Services.CustomerSegmentDto>> GetCustomerSegments(Guid restaurantId)
        {
            var userId = GetCurrentUserId();
            return await _notificationService.GetCustomerSegmentsAsync(restaurantId, userId);
        }

        /// <summary>
        /// Get notification campaigns for a restaurant
        /// </summary>
        [HttpGet("restaurants/{restaurantId}/notification-campaigns")]
        public async Task<Application.Contracts.Dtos.PagedResultDto<NotificationCampaignDto>> GetNotificationCampaigns(Guid restaurantId, [FromQuery] Application.Contracts.Dtos.PagedAndSortedResultRequestDto input)
        {
            var userId = GetCurrentUserId();
            return await _notificationService.GetNotificationCampaignsAsync(restaurantId, input, userId);
        }

        /// <summary>
        /// Get notification campaign details
        /// </summary>
        [HttpGet("notification-campaigns/{campaignId}")]
        public async Task<NotificationCampaignDto> GetNotificationCampaign(Guid campaignId)
        {
            var userId = GetCurrentUserId();
            return await _notificationService.GetNotificationCampaignAsync(campaignId, userId);
        }

        /// <summary>
        /// Get notification analytics for a campaign
        /// </summary>
        [HttpGet("notification-campaigns/{campaignId}/analytics")]
        public async Task<NotificationAnalyticsDto> GetNotificationAnalytics(Guid campaignId)
        {
            var userId = GetCurrentUserId();
            return await _notificationService.GetNotificationAnalyticsAsync(campaignId, userId);
        }

        /// <summary>
        /// Cancel a scheduled notification campaign
        /// </summary>
        [HttpPost("notification-campaigns/{campaignId}/cancel")]
        public async Task<bool> CancelNotificationCampaign(Guid campaignId)
        {
            var userId = GetCurrentUserId();
            return await _notificationService.CancelNotificationCampaignAsync(campaignId, userId);
        }

        /// <summary>
        /// Get notification statistics for restaurant dashboard
        /// </summary>
        [HttpGet("restaurants/{restaurantId}/notification-statistics")]
        public async Task<RestaurantNotificationStatsDto> GetNotificationStatistics(Guid restaurantId, [FromQuery] DateTime? startDate = null, [FromQuery] DateTime? endDate = null)
        {
            var userId = GetCurrentUserId();
            return await _notificationService.GetNotificationStatisticsAsync(restaurantId, startDate, endDate, userId);
        }
        ...
        /// </summary>
        [HttpPost("restaurants/{restaurantId}/notification-templates")]
        public async Task<NotificationTemplateDto> CreateNotificationTemplate(Guid restaurantId, [FromBody] CreateNotificationTemplateDto dto)
        {
            var userId = GetCurrentUserId();
            dto.RestaurantId = restaurantId;
            return await _notificationService.CreateNotificationTemplateAsync(dto, userId);
        }

        /// <summary>
        /// Get notification templates for a restaurant
        /// </summary>
        [HttpGet("restaurants/{restaurantId}/notification-templates")]
        public async Task<List<NotificationTemplateDto>> GetNotificationTemplates(Guid restaurantId)
        {
            var userId = GetCurrentUserId();
            return await _notificationService.GetNotificationTemplatesAsync(restaurantId, userId);
        }

        /// <summary>
        /// Update notification template
        /// </summary>
        [HttpPut("notification-templates/{templateId}")]
        public async Task<NotificationTemplateDto> UpdateNotificationTemplate(Guid templateId, [FromBody] UpdateNotificationTemplateDto dto)
        {
            var userId = GetCurrentUserId();
            return await _notificationService.UpdateNotificationTemplateAsync(templateId, dto, userId);
        }

        /// <summary>
        /// Delete notification template
        /// </summary>
        [HttpDelete("notification-templates/{templateId}")]
        public async Task<bool> DeleteNotificationTemplate(Guid templateId)
        {
            var userId = GetCurrentUserId();
            return await _notificationService.DeleteNotificationTemplateAsync(templateId, userId);
        }

        /// <summary>
        /// Send notification using template
        /// </summary>
        [HttpPost("notification-templates/{templateId}/send")]
        public async Task<NotificationCampaignDto> SendNotificationFromTemplate(Guid templateId, [FromBody] CustomerTargetingCriteria? customTargeting = null)
        {
            var userId = GetCurrentUserId();
            return await _notificationService.SendNotificationFromTemplateAsync(templateId, customTargeting, userId);
        }
